...
**Database schema:** Docker Compose does **not** run Alembic automatically. For localhost, `run-m2-bioresearch-import.sh` and `run-m2-bioresearch-link.sh` run `alembic upgrade head` in the `backend` container (creates `patient_records` and other tables). Disable with `SHOWCASE_M2_BRA_RUN_ALEMBIC=0` if you manage migrations yourself.
